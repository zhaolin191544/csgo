...
<!--
我们可以将这两者的缺陷都归纳为因为成本和维护从而无法进行大规模的部署。因此 我们需要去寻找一种更加普适并且便宜的方案，由此 论文提出了GPSoil的解决方案
-->
...
<!--
第一个挑战是传播误差巨大，因为距离，多普勒 时钟漂移等等因素，都会将及其微小的土壤水分变化信号给淹没。第二是我们需要感知的土壤深度会更进一步地去削弱接收到的信号。第三十GNSS以码相测距，含水率地分辨率小于GNSS的分辨率。观察下面这张图，图a说明接收机报告的码相几乎完全被“卫星运动”主导，呈现稳定线性增长，右图根据数学公式推导，展示的 relative code phase 把“卫星运动”这个大因素扣掉了，剩下的才是我们真正关心的、和土壤有关的精细变化。这些“起伏”就包含了天线切换、土壤介质差异、接收机时钟漂移等本地效应。这些对比告诉我们，必须先把“卫星运动带来的大变化”从码相里减掉，才能看到天线/土壤造成的那一点点细微变化。。
-->
...
首先，GPSoil 是首次利用 GNSS 信号来做土壤水分感知的系统，它最大的突破就是完全不需要自建无线发射端，只依赖本来就存在的卫星信号。
在硬件上，它用一颗 GNSS 芯片配合两个天线，通过 RF 开关实现差分测量，把卫星到地面的超长路径误差都抵消掉。
在软件上，它把原本会影响定位精度的时钟漂移反过来当成工具，突破了商用芯片 14 cm 的分辨率限制。
